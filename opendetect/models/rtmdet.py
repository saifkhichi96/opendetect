...
class RTMDetModel:
    ...
    def preprocess_rgb_frame(
        ...
        # Convert RGB -> BGR to match RTMDet ONNX normalization convention.
        ...
        padded[:resized_h, :resized_w] = resized_bgr
    ...
    def _decode_if_needed(self, prediction: np.ndarray) -> np.ndarray:
        ...
        decoded = prediction.copy()[None, ...]
        if decoded.shape[1] == grids.shape[1]:
    ...
    def postprocess(
        ...
        for batch_idx in range(raw.shape[0]):
            ...
            decoded = self._decode_if_needed(prediction)
            if decoded.shape[-1] < 6:
                results.append(self._empty_detections())
                continue

            boxes = decoded[:, :4]
            scores_matrix = decoded[:, 4:5] * decoded[:, 5:]
